 ...
 *
 */
class SeoModuleDataService extends DataServiceBase {

    const TABLE_NAME = 'seo_module';//对应数据库表
    const PRIMARY_KEY = 'module_id'; //对应主键，如果有
    const PV_REAL = 2;
    const LIKE_INIT = 3;
     ...
     * @return array|mixed
     */
    public function getModuleList($where_condition, $limit = NULL, $columns = NULL, $order = NULL){
        $data=$this->getList($where_condition, self::TABLE_NAME, $limit, $columns, $order);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取模块信息总数
     * @param $where_condition 查询条件
     * @return array|mixed
     */
    public function getModuleTotal($where_condition){
        $data=$this->getTotalBy($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取一条模块信息
     * @param $where_condition 查询条件
     * @return bool|mixed
     */
    ...
    /**
     * @purpose 根据主键获取一条模块信息
     * @param $id 编号
     * @return bool|mixed
     */
    public function getOneById($id){
        $where_condition=array('module_id'=>"=".$id);
        $data=$this->getOne($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }
    
    /**
     * @purpose 根据名称获取一条模块信息
     * @param $name 名称
     * @return bool|mixed
     */
    public function getOneByName($name){
        if(!$name) return false;
        $where_condition=array('module_name'=>"='".$name."'");
