 ...
 *
 */
class SemCampaignBaseDataService extends DataServiceBase {

    const TABLE_NAME = 'sem_campaign';//对应数据库表
    const PRIMARY_KEY = 'campaignId'; //对应主键，如果有
    const PV_REAL = 2;
    const LIKE_INIT = 3;
     ...
     * @return array|mixed
     */
    public function getCampaignList($where_condition, $limit = NULL, $columns = NULL, $order = NULL){
        $data=$this->getList($where_condition, self::TABLE_NAME, $limit, $columns, $order);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取推广计划信息总数
     * @param $where_condition 查询条件
     * @return array|mixed
     */
    public function getCampaignTotal($where_condition){
        $data=$this->getTotalBy($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取一条推广计划信息
     * @param $where_condition array 查询条件
     * @return bool|mixed
     */
    public function getOneCampaign($where_condition){
        $data=$this->getOne($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }

    /**
     * @purpose 根据主键获取一条推广计划信息
     * @param $id 编号
     * @param $platform 所属平台，默认1为百度
     * @return bool|mixed
     */
