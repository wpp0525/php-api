 ...
 *
 */
class SemMonitorBaseDataService extends DataServiceBase {

    const TABLE_NAME = 'sem_monitor';//对应数据库表
     ...
     * @return array|mixed
     */
    public function getMonitorList($where_condition, $limit = NULL, $columns = NULL, $order = NULL){
        $data=$this->getList($where_condition, self::TABLE_NAME, $limit, $columns, $order);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取推广监控信息总数
     * @param $where_condition 查询条件
     * @return array|mixed
     */
    public function getMonitorTotal($where_condition){
        $data=$this->getTotalBy($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }

    /**
     * @purpose 根据条件获取一条推广监控信息
     * @param $where_condition 查询条件
     * @return bool|mixed
     */
    public function getOneMonitor($where_condition){
        $data=$this->getOne($where_condition, self::TABLE_NAME);
        return $data?$data:false;
    }

    /**
     * @purpose 根据主键获取一条推广监控信息
     * @param $id 编号
     * @return bool|mixed
     */
