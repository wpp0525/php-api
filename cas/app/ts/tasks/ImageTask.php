...
class ImageTask extends Task {
    ...
    public function setDI(Phalcon\DiInterface $dependencyInjector) {
        ...
        $this->svc = new \ImageWorkerService($dependencyInjector);
    }

    /**
     * @example php ts.php image destImageRelation start|stop|restart
     */
    public function destImageRelationAction(array $params) {
        $config = $this->getDI ()->get ( 'config' )->daemon->image->imagedest->toArray ();
        $default_daemon_options = array (
            'appName'               => 'image-dest',
            'appDir'                => __DIR__ . '/../',
            'appDescription'        => 'CAS Dest Image Relation Worker Service',
            'logLocation'           => __DIR__ . '/../logs/image-dest/daemon.log',
            'authorName'            => 'System Daemon',
            'authorEmail'           => 'root@127.0.0.1',
            'appPidLocation'        => __DIR__. '/../run/image-dest/daemon.pid',
