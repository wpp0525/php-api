...
class BbsToTravelWorkerService implements DaemonServiceInterface
{
    ...
    private function insertData()
    {
        ...
        foreach ($this->getRows($tid_data) as $row) {
     ...
     * 获取所有符合迁移条件的数据ID
     * 迁移条件：
     * 版块：游记攻略、旅游指南、光影记录、自驾旅游
     * 图章：精华、热帖、美图、优秀、置顶、推荐、原创、版主推荐、爆料、编辑采用
     * 将BBS全部数据中归属于以上4个版块中的且带有以上至少一个图章的文章，迁移至游记数据库中。
     * @return array
     */
    private function getAllowTid()
    {
        $params = array(
            'table' => 'forum_thread',
            'select' => 'tid,authorid',
            'where' => array('fid' => array('IN', "('5','2','4','129')"), 'stamp' => array('NEQ', '-1'), 'authorid' => array('NEQ', '1')),
        );
        if ($this->flag_id)
