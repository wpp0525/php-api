 ...
 *
 */
class GoodsPoolRedisV2DelService implements DaemonServiceInterface
{
    ...
    public function __construct($di)
    {
        ...
        $this->goodspoolv2Redis = $di->get('goodspoolredis');
    }
...
//            $this->productpoolv2Redis->hMset($this->product_pool_key_v2 . $product_infos_value['PRODUCT_ID'], $insert_data);
            $this->goodspoolv2Redis->expire($this->product_pool_key_v2 . $last_id, 0 ); //过期时间1个月
