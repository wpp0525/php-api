 ...
 *
 */
class DestinfoController extends ControllerBase {
    ...
    public function destAllInfoAction(){
        ...
        $redis_key = str_replace('{dest_id}', $dest_id, RedisDataService::REDIS_NEW_DEST_ALL_INFO);
        $ttl=$this->redisConfig['ttl']['lvyou_dest_new_dest_all']?$this->redisConfig['ttl']['lvyou_dest_new_dest_all']:14400;
        $this->redis_svc->dataHmset($redis_key,$dest_info,$ttl);
...
//    }
    ...
    public function destIndexImageListAction(){
        $dest_id=intval($this->dest_id);
        $num=intval($this->num);
        $elite_img=$this->dest_image_svc->getImgById($dest_id,$num);
    ...
    public function destSubjectListAction(){
        ...
        $subject_list=array();
        $viewspot_ids=$this->getViewSpotIds($base_id);
        if(!$viewspot_ids) {$this->jsonResponse($subject_list);die();}
        $viewspot_ids_str=implode(',',$viewspot_ids);
        $subject_list=$this->mo_subject->getDestSubList($viewspot_ids_str);
    ...
    public function getRecomDestTopAction(){
        $dest_id = $this->dest_id;
        if(!$dest_id) $this->jsonResponse(array());
        $find_type = $this->find_type ? $this->find_type : '';
        $limit = $this->num ? $this->num : 3 ;
        $scenicviewspot = $this->di->get('cas')->get('scenicviewspot-data-service');
        $result = $scenicviewspot->getRecomDestTop($dest_id, $find_type, $limit);

        if($result && is_array($result)){
            $dest_image_svc = $this->di->get('cas')->get('dest_image_service');
            foreach($result as $key => $val){
                if($val['img_url'] == ''){
                    $result[$key]['img_url'] = $dest_image_svc->getCoverByDestId($val['dest_id']);
                }
            }
        }
    ...
    public function getRecomMainDestTopAction(){
        $dest_id = $this->dest_id;
        if(!$dest_id) $this->jsonResponse(array());
        $find_type = $this->find_type ? $this->find_type : '';
        $limit = $this->num ? $this->num : 3 ;
        $scenicviewspot = $this->di->get('cas')->get('scenicviewspot-data-service');
        $result = $scenicviewspot->getRecomDestTop($dest_id, $find_type, $limit, 'MAIN_DEST');
